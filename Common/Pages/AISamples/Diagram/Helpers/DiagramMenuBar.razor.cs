...
#pragma warning disable CS8618
        ...
        /// </summary>
        public bool enablePasteButten { get; set; }
        /// <summary>
        ...
        /// </summary>
        internal TextToMindMap Parent { get; set; }
        public bool IsDuplicate { get; set; }
        public bool IsJsonLoading { get; set; }
#pragma warning restore CS8618
        ...
        internal List<ContextMenuItemModel> WindowMenuItems = new List<ContextMenuItemModel>
    {
        ...
        private async Task MenuClick(Syncfusion.Blazor.Navigations.MenuEventArgs<ContextMenuItemModel> args)
        {
            ...
            string commandType = args.Item.Text!.Replace(" ", "", StringComparison.Ordinal);
            switch (commandType.ToLowerInvariant())
            {
                case "new":
                    ...
                    await Parent.Toolbar.HideElements("hide-toolbar", true).ConfigureAwait(false);
                ...
                case "open":
                    ...
                    await OpenUploadBox(true, ".json").ConfigureAwait(false);
                    IsJsonLoading = false;
                    break;
                case "undo":
                    Edit = true;
                ...
                case "delete":
                    ...
                    await diagram.EndUpdateAsync().ConfigureAwait(false);
                    break;
                case "duplicate":
                ...
                case "saveas":
                    ...
                    await SaveDialogBox.SaveDialog.ShowAsync().ConfigureAwait(false);
                    break;
                case "print":
                    File = true;
                    await PrintDialogBox.PrintDialog.ShowAsync().ConfigureAwait(false);
                    break;
                case "export":
                    File = true;
                    await ExportDialogBox.ExportDialog.ShowAsync().ConfigureAwait(false);
                    break;
                case "showtoolbar":
                    Window = true;
                    WindowMenuItems[0].IconCss = WindowMenuItems[0].IconCss == "sf-icon-Selection" ? "sf-icon-Remove" : "sf-icon-Selection";
                    // Toggle the toolbar visibility on the toolbar component and notify it to re-render.
                    if (Parent.Toolbar != null)
                    {
                        Parent.Toolbar.ToolbarVisible = !Parent.Toolbar.ToolbarVisible;
                        Parent.Toolbar.StateChanged();
                    }
                    break;
                case "showshortcuts":
            ...
            Dictionary<string, bool> Select = new Dictionary<string, bool>()
{
        ...
        private static void OnUploadSuccess(SuccessEventArgs args)
        {
        ...
        private async Task OnUploadFileSelected(Syncfusion.Blazor.Inputs.UploadingEventArgs args)
        {
            ...
            if (args.FileData?.Type == "json")
            {
                ...
                string json = await FileUtil.LoadFile(jsRuntime, args.FileData).ConfigureAwait(false);
                json = json.Replace(System.Environment.NewLine, string.Empty, StringComparison.Ordinal);
                await Parent.Diagram!.LoadDiagramAsync(json.ToString()).ConfigureAwait(false);
                IsJsonLoading = false;
            }
            else
            {
        ...
        public async Task ExportDiagram(Syncfusion.Blazor.Diagram.DiagramExportSettings exportOptions, string exportType, string fileName)
        {
            ...
            var images = await diagram!.ExportAsync(exportFormat, exportOptions).ConfigureAwait(false);
            var Orientation = PdfPageOrientation.Landscape;
            if (string.Equals(exportType, "PDF", StringComparison.Ordinal))
            {
                await ExportToPdf(fileName, Orientation, true, images!).ConfigureAwait(false);
            }
            else
            {
                exportFormat = Enum.Parse<DiagramExportFormat>(exportType, true);
                await diagram.ExportAsync(fileName, exportFormat, exportOptions).ConfigureAwait(false);
            }
        }
        /// <summary>
...
#pragma warning restore CA2000
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                ...
                if (allowDownload)
                {
                    await JSRuntimeExtensions.InvokeAsync<string>(jsRuntime, "downloadPdf", new object[] { base64String, fileName }).ConfigureAwait(false);
        ...
        public async Task PrintDiagram(Syncfusion.Blazor.Diagram.DiagramPrintSettings printOptions)
        {
            ...
            await diagram!.PrintAsync(printOptions).ConfigureAwait(false);
        }
        /// <summary>
        ...
        public bool EnableMenuItems(string itemText, ObservableCollection<object> collection)
        {
            ...
            {
                var commandType = itemText.Replace(" ", "", StringComparison.Ordinal);
