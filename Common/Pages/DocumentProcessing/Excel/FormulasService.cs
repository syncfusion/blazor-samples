...
namespace BlazorDemos.Data.FileFormats.XlsIO
{
    public class FormulasService
    {
        ...
        public FormulasService(Dictionary<string, MemoryStream> fileData)
        {
        ...
        public MemoryStream FormulasXlsIO(string button, out double computedValue, out string formulaString)
        {
            computedValue = 0.0;
            ...
            else
            {
                ...
                using (ExcelEngine excelEngine = new ExcelEngine())
                {
                    ...
                    IWorkbook workbook = application.Workbooks.Open(fileDataValue["formula-template.xls"]);

                    //The first worksheet object in the worksheets collection is accessed
                    IWorksheet sheet = workbook.Worksheets[0];

                    //Read computed Formula Value
                    computedValue = sheet.Range["C1"].FormulaNumberValue;
