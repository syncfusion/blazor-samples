...
namespace BlazorDemos.Data.DocumentProcessing.Word
{
    public class BookmarkNavigationService
    {
        ...
        public BookmarkNavigationService(Dictionary<string, MemoryStream> fileData)
        {
        ...
        public MemoryStream BookmarkNavigation(string documentType,string button)
        {
            ...
            {
                return fileDataValue["bookmark-template.docx"];
            }
            ...
            nwdInformation.Open(fileDataValue["bookmark-template.docx"], FormatType.Doc);
            // Open an existing template document with multiple sections to get Northwind data.
            WordDocument templateDocument = new WordDocument();
            templateDocument.Open(fileDataValue["bkmk-document-part-template.docx"], FormatType.Doc);
            ...
            pic.LoadImage(fileDataValue["northwind.png"]);
            pic.WidthScale = 50f;  // It reduces the image size because it doesn't fit 
