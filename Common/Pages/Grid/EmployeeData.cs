...
namespace BlazorDemos.Pages.Grid.EmployeeDataNameSpace
{
    ...
    public static class EmployeeDataSource
    {
        public static string GetMail(string first, string last, int index)
        {
            ...
            string local = string.Concat($"{first}.{last}".Select(char.ToLowerInvariant)).Replace(" ", "", StringComparison.Ordinal);

            return $"{local}{number}@sample.com";
        }

        public static IReadOnlyList<EmployeeRecord> GetAllRecords()
        {
