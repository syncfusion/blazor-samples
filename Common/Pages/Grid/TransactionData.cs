...
namespace BlazorDemos.Pages.Grid.TransactionDataNameSpace
{
    /// <summary>
    ...
    public static class TransactionData
    {
        ...
        public static IReadOnlyList<TransactionRecord> GetAllRecords()
        {
        ...
        private static double GetTransactionAmount(string description, int seed)
        {
            ...
            return description switch
            {
                ...
                string s when s.Contains("Subscription", StringComparison.Ordinal) || s.Contains("Monthly", StringComparison.Ordinal)
                    => Math.Round(299.99d + (seed % 500), 2),

                // High-value hardware purchases (~$2,500-10,000)
                string s when s.Contains("Hardware", StringComparison.Ordinal) || s.Contains("Equipment", StringComparison.Ordinal)
                    => Math.Round(2500.0d + (seed % 7500), 2),

                // Professional service fees (~$1,500-10,000)
                string s when s.Contains("Consulting", StringComparison.Ordinal) || s.Contains("Professional", StringComparison.Ordinal)
                    => Math.Round(1500.0d + (seed % 8500), 2),

                // Refund amounts (~$100-5,100)
                string s when s.Contains("Refund", StringComparison.Ordinal)
                    => Math.Round(100.0d + (seed % 5000), 2),

                // Tax adjustments (~$500-3,500)
                string s when s.Contains("Tax", StringComparison.Ordinal)
                    => Math.Round(500.0d + (seed % 3000), 2),

                // Insurance premiums (~$750-4,750)
                string s when s.Contains("Insurance", StringComparison.Ordinal) || s.Contains("Premium", StringComparison.Ordinal)
                    => Math.Round(750.0d + (seed % 4000), 2),

                // License renewal fees (~$400-3,400)
                string s when s.Contains("License", StringComparison.Ordinal)
                    => Math.Round(400.0d + (seed % 3000), 2),

                // Training/workshop fees (~$800-3,300)
                string s when s.Contains("Training", StringComparison.Ordinal)
                    => Math.Round(800.0d + (seed % 2500), 2),

                // Cloud and support services (~$250-1,750)
                string s when s.Contains("Cloud", StringComparison.Ordinal) || s.Contains("Support", StringComparison.Ordinal)
