...
namespace BlazorDemos.Pages.Grid
{
    ...
    public static class FlightBookingData
    {
        ...
        /// </summary>
        public static IReadOnlyList<FlightBookingRecord> GetAllRecords()
        {
            ...
            for (int i = 0; i < 10000; i++)
            {
                ...
                string emailPrefix = string.Concat(passengerName.Replace(" ", ".", StringComparison.Ordinal).Select(char.ToLowerInvariant));
                string email = emailPrefix + emailDomains[(i * 5) % emailDomains.Length];

                // Flight number format (airline code + 4-digit number).
                string flightNumber = airlineCode + (1000 + (i % 9000)).ToString(CultureInfo.InvariantCulture);

                // Seat number (row 1-100 + letter A-F).
                int row = (i % 100) + 1;
                char seatLetter = seatLetters[(i * 2) % seatLetters.Length];
                string seatNumber = row.ToString(CultureInfo.InvariantCulture) + seatLetter;
