...
{
    public sealed class ColumnValidator : ComponentBase, IDisposable
    {
        [Parameter]
        ...
        private void ValidateField(FieldIdentifier id)
        {
            ...
            void AddError(string field, string message)
            {
                ...
                Context?.ShowValidationMessage?.Invoke(field, false, message);
            }
            void Clear(string field) => Context?.ShowValidationMessage?.Invoke(field, true, string.Empty);
                ...
                case nameof(TaskModel.Progress):
                    {
                        ...
                        if (predecessorIds.Count > 0)
                        {
                            var unfinishedPredecessors = predecessorIds
                                .Where(pid =>
                                {
                                    var parentTask = ColumnValidation.GanttInstance.GetTaskByID(pid.ToString(CultureInfo.InvariantCulture));
                ...
                case nameof(TaskModel.TeamSize):
                    {
                        ...
                        var priority = task.Priority?.Trim();
                        var (min, max) =
                            string.Equals(priority, "Low", StringComparison.OrdinalIgnoreCase) ? (1, 6) :
                            string.Equals(priority, "Medium", StringComparison.OrdinalIgnoreCase) ? (2, 10) :
                            string.Equals(priority, "High", StringComparison.OrdinalIgnoreCase) ? (3, 12) :
                            string.Equals(priority, "Critical", StringComparison.OrdinalIgnoreCase) ? (4, 15) :
                            (1, 15);
                ...
                case nameof(TaskModel.Predecessor):
                    {
                        ...
                        var predecessors = predecessorIds
                            .Select(pid => ColumnValidation.GanttInstance.GetTaskByID(pid.ToString(CultureInfo.InvariantCulture)))
                            .Where(dep => dep != null)
                            .ToList();
                        ...
                        // Validation Rules
                        if (notAtHundred.Count > 0 && task.Progress > 0)
                        {
                            AddError(fieldName, $"Progress must be 0% until predecessor(s) complete: {string.Join(",", notAtHundred)}.");
                            break;
                        }
                        if (string.Equals(task.Status, "Completed", StringComparison.OrdinalIgnoreCase) && notCompletedStatus.Count > 0)
                        {
                        ...
                        if ((string.Equals(task.Status, "In Progress", StringComparison.OrdinalIgnoreCase) ||
                             string.Equals(task.Status, "Review", StringComparison.OrdinalIgnoreCase)) && notAtHundred.Count > 0)
                        {
                            AddError(fieldName, $"Cannot proceed until predecessor(s) reach 100%: {string.Join(",", notAtHundred)}.");
                            break;
                        }
                        if (string.Equals(task.Priority, "Critical", StringComparison.OrdinalIgnoreCase) && notCompletedStatus.Count > 0)
                        {
                        ...
                        if ((string.Equals(task.Priority, "High", StringComparison.OrdinalIgnoreCase) ||
                             string.Equals(task.Priority, "Critical", StringComparison.OrdinalIgnoreCase)) && overBudget.Count > 0 && task.Progress > 0)
                        {
        ...
        /// </returns>
        private static bool IsDevelopmentTask(TaskModel task)
        {
