...
namespace BlazorDemos.Pages.TreeGrid
{
    ...
    {
        public static IReadOnlyList<BomItem> GetBomData()
        {
            ...
            void AddOrIncrement(string parentId, string name, int qty, decimal cost, int lead)
            {
                ...
                // ✅ Deterministic index (based on id + name)
                int index = (id + name.Length) % 1000;

                // ✅ Deterministic variation (0.9 - 1.1 approx)
                decimal variation = 0.9m + ((index % 200) / 1000m);
                var finalCost = Math.Round(cost * variation, 2);

                // ✅ Deterministic lead adjustment (0.8 - 1.2 approx)
                double randomFactor = 0.8 + ((index % 400) / 1000.0);
                var finalLead = Math.Max(2, (int)Math.Round(lead * randomFactor));
        ...
        public static IReadOnlyList<BomItem> GetBomDataStatic()
        {
            var data = new List<BomItem>();
            int id = 1000;
