...
namespace BlazorDemos.Pages.TreeGrid.TreeVirtualDataSource
{

    public class SelfReferenceData
    {

        private static readonly List<SelfReferenceData> tree = new();

        public static IList<SelfReferenceData> TreeData => tree;
        ...
        public static IReadOnlyList<SelfReferenceData> GetTree()
        {
            if (tree.Count == 0)
            {
                int root = -1;
                for (var t = 1; t <= 10000; t++)
                {
                    ...
                    tree.Add(new SelfReferenceData() { TaskID = rootItem, TaskName = "Parent task " + rootItem.ToString(CultureInfo.InvariantCulture), StartDate = new DateTime(2021, 06, 07), EndDate = new DateTime(2023, 08, 25), IsParent = true, ParentItem = null, Progress = progr, Priority = math, Duration = duration});
                    ...
                    for (var c = 0; c < 2; c++)
                    {
                        ...
                        tree.Add(new SelfReferenceData() { TaskID = iD, TaskName = "Child task " + iD.ToString(CultureInfo.InvariantCulture), StartDate = new DateTime(2021, 06, 07), EndDate = new DateTime(2023, 08, 25), IsParent = (((parent + c + 1) % 3) == 0), ParentItem = rootItem, Progress = progr, Priority = val, Duration = duration });
                        if ((((parent + c + 1) % 3) == 0))
                        {
                            ...
                            for (var s = 0; s < 2; s++)
                            {
                                ...
                                tree.Add(new SelfReferenceData() { TaskID = tree.Count + root + 1, TaskName = "Sub task " + (tree.Count + root + 1).ToString(CultureInfo.InvariantCulture), StartDate = new DateTime(2021, 06, 07), EndDate = new DateTime(2023, 08, 25), IsParent = false, ParentItem = iD, Progress = (immParent % 2 == 0) ? "In Progress" : "Closed", Priority = Prior, Duration = duration });
                            }
                        }
                    }
                }
    ...
    public class VirtualData
    {
        ...
        public static IReadOnlyList<VirtualData> GetTreeVirtualData()
        {
