...
namespace TreeShipmentdata
{
    public class ShipmentData
    {
        ...
        public string? OrderReached { get; set; }
        ...
        public static IReadOnlyList<ShipmentData> GetShipmentData()
        {
        ...
        public static IReadOnlyList<ShipmentData> GetData()
        {
            ...
            for (int p = 1; p <= 40; p++)
            {
                ...
                for (int c = 0; c < 4; c++)
                {
                    ...
                    // ✅ Deterministic values (NO Random / RNG)
                    int units = 20 + ((p + c * 2) % 11);          // 20–30
                    int unitPrice = 5 + ((p * 2 + c * 3) % 20);   // 5–24
                    int price = units * unitPrice;

                    // Dates (deterministic)
                    var orderDate = baseOrderDate.AddDays(p * 7 + c * 3);
                    int shipLag = 3 + ((p + c) % 10);             // 3–12 days
                    var shippedDate = orderDate.AddDays(shipLag);

                    // Delivery status (deterministic)
                    var reached = shippedDate >= orderDate.AddDays(7) && ((p + c) % 3 != 0)
                        ? "Yes"
                        : "No";
