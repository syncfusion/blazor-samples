...
namespace BlazorDemos.Pages.TreeGrid.TreeGrid
{
    public partial class ObservableBinding
    {
        ...
        private int? SelectedParentTicketId;
        ...
        private SupportTicketData? SelectedRecord;
        ...
        private async void ResolveRandomTicket()
        {
            ...
            var selectedTicket = await TreeGridRef!.GetSelectedRecordsAsync().ConfigureAwait(true);
        ...
        private void AddTicketWithParent()
        {
            ...
            // ✅ Deterministic index based on newId
            int index = newId % 10;

            // Default selections
            string selectedCategory = "Technical";
            string selectedCustomer = customers[index % customers.Length];
            ...
            var newTicket = new SupportTicketData(
                ...
                "Open",
                agents[index % agents.Length], // ✅ deterministic agent
                selectedCustomer,
                DateTime.Now,
                DateTime.Now.AddHours(4 + (index % 45)), // ✅ 4–48 hours deterministic
                2 + (index % 14) // ✅ 2–15 deterministic
            );
        ...
        private async void NavigateToParentTicketPage(int parentTicketId, int addedId)
        {
            ...
            if (selectInd > -1)
            {
                await TreeGridRef!.ScrollIntoViewAsync((int)selectInd).ConfigureAwait(true);
                await TreeGridRef!.SelectRowAsync((int)selectInd).ConfigureAwait(true);
            }
        }

        private static int GenerateFiveDigitTicketId(int currentMaxId)
        {
