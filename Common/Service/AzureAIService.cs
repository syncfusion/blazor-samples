    ...
    public class AzureAIService
    {
        ...
        public async Task<string> GetChatResponseAsync(ChatParameters options)
        {
            ...
            try
            {
                ...
                ModerationClient moderationClient = client.GetModerationClient("omni-moderation-latest");
                ModerationResult result = await moderationClient.ClassifyTextAsync(options.Messages.Last().Text);
                if (result.Flagged)
                {
                    return "The submitted input could not be processed because it violates our content guidelines. As a result, the response has been moderated. Please revise your input and try again.";
                }
