...
using System.Linq;
using BlazorDemos.Pages.TreeGrid.TreeVirtualDataSource;
...
namespace BlazorDemos.Controllers
{
    ...
    public class TreeGridController : ControllerBase
    {
        ...
        public object Get(int? code)
        {
            ...
            if (SelfReferenceData.TreeData.Count == 0)
            ...
            if (req.filter != "" && !req.filter.Contains("null"))
            {
                ...
                IQueryable<SelfReferenceData> data1 = SelfReferenceData.TreeData.Where(f => f.ParentItem == fltr).AsQueryable();
                return new { result = data1.ToList(), items = data1.ToList(), count = data1.Count() };
            }
            List<SelfReferenceData> data = SelfReferenceData.TreeData.ToList();
