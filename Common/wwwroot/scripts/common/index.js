...
window.treeKeyboard = {
    move: function (down) {
        const items = Array.from(
            document.querySelectorAll('[role="treeitem"]:not([data-hidden="true"])')
        );
        ...
        // Moving down past last visible item
        if (down && index === items.length - 1) {
            window.dispatchEvent(new CustomEvent("tree-reach-end"));
            return;
        }

        const target = down ? items[index + 1] : items[index - 1];
...
window.treeFocus = {
    ...
    getFocusedSelector: function () {
        const el = document.activeElement;
        if (!el) return null;

        if (el.dataset.treeId) {
            return `[data-tree-id='${el.dataset.treeId}']`;
        }
        if (el.dataset.sampleUrl) {
            return `[data-sample-url='${el.dataset.sampleUrl}']`;
        }
        return null;
    }
};
...
window.focusTreeItem = function (name) {
    const el = document.querySelector(`[data-tree-id="${name}"]`);
    if (el) {
        el.focus({ preventScroll: true });
    }
};
...
window.resetToastTargetPosition = function (id) {
    const element = document.getElementById(id);
    if (element) {
...
// Phase 1: Adjust control section width only
function adjustControlSectionWidth(controlClass, isCollapsed, initialLoad = false) {
    const leftPanel = document.querySelector("." + controlClass);
    
    if (!initialLoad) {
        leftPanel.style.transition = "width 200ms ease-in-out";
    }
    ...
    // Return a promise that resolves after 200ms
...
// Phase 2: Adjust property panel width and show/hide content
function adjustPropertyPanelWidth(isCollapsed, initialLoad = false) {
    ...
    const propertyPanelTitle = document.querySelector(".Property-panel-title, .Property-panel-title-collapsed");
    
    if (!initialLoad) {
        propertyPanel.style.transition = "width 200ms ease-in-out";
    }
    
    if (propertyPanelContent && propertyPanelTitle) {
        propertyPanelContent.style.visibility = isCollapsed ? 'hidden' : 'visible';
    }

    if (propertyPanelTitle) {
        propertyPanelTitle.style.opacity = "0";
    }

    if (propertyPanel) {
        propertyPanel.style.width = isCollapsed ? '5.6%' : '34.33333333%';
    }
    ...
    return new Promise(resolve => {
        setTimeout(() => {
            if (propertyPanelTitle) {
                propertyPanelTitle.style.opacity = "1";
            }
            resolve(true);
        }, 200);
    });
