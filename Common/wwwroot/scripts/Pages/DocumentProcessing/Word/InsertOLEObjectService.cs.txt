...
namespace BlazorDemos.Data.DocumentProcessing.Word
{
    public class InsertOLEObjectService
    {
        ...
        public MemoryStream InsertOLEObject(string documentType)
        {
            ...
            if (documentType == "DOC")
            {
                fileStream = new FileStream(basePath + @"/data/word/ole-template.doc", FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
                //Open an existing word document                 
                oleSource = new WordDocument(fileStream, FormatType.Doc);
            }
            else
            {
                fileStream = new FileStream(basePath + @"/data/word/ole-template.docx", FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
                //Open an existing word document 
