...
namespace BlazorDemos.Shared
{
    ...
    public class SampleService
    {
...
#else
            "_content/BlazorServerCommon_NETCORE3_1/";
#endif
#endif
            ...
            if (updatedUrl != string.Empty)
            {
                ...
                if (splittedUrl.Length >= 2)
                {
                    try
                    {
                        ...
                        if (categoryName == "buttons" && sampleName != "default-functionalities" || categoryName == "colorpicker" && sampleName == "inline")
                        {
        ...
        {
            // Handle condition to prevent a culture switching from breaking.
            currentUrl = Uri.UnescapeDataString(currentUrl);
            currentUrl = currentUrl.IndexOf('&') != -1 ? currentUrl.Split("&")[1].TrimEnd('/').Replace("//", "/") : currentUrl.TrimEnd('/').Replace("//", "/");
