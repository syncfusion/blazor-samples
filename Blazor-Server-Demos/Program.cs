...
using AIAssistViewAzureAI.Components.Services;
...
// Set the resx file folder path to access
            ...
            var supportedCultures = new[] { "en-US", "de-DE", "fr-CH", "zh-CN","ar" };
    ...
    #region Localization
        ...
        // Apply currency override from cookie (e.g., when neutral 'ar' selects AED)
        app.Use(async (context, next) =>
        {
            try
            {
                var currency = context.Request.Cookies["currency"];
                if (!string.IsNullOrEmpty(currency))
                {
                    var current = System.Globalization.CultureInfo.CurrentCulture;
                    var clone = new System.Globalization.CultureInfo(current.Name);
                    var nf = (System.Globalization.NumberFormatInfo)clone.NumberFormat.Clone();
                    nf.CurrencySymbol = currency;
                    clone.NumberFormat = nf;
                    System.Globalization.CultureInfo.CurrentCulture = clone;
                    System.Globalization.CultureInfo.CurrentUICulture = clone;
                }
            }
            catch
            {
